...
contract KYCVerifierTest is Test {
    ...
    /// @dev Test helper: most tests call verifyKYC directly as the user (vm.prank(user)),
    ///      so `caller` in the signed payload is implicitly `user` (since KYCVerifier uses caller = msg.sender).
