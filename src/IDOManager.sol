...
contract IDOManager is IIDOManager, ReentrancyGuard, WithKYCVerifier, ReservesManager {
    ...
    */
    ...
    function setIdoTime(
        ...
        _validateIdoId(idoId);
        _validateMutableIDOConfig(idoId);
        if (_idoStartTime >= _idoEndTime) revert InvalidIDOTimeRange();
    ...
    function setTwapPriceUsdt(
        ...
        _validateIdoId(idoId);
        if (twapPriceUsdt == 0) revert InvalidPrice();
    ...
    */
    ...
    function _validateIDOInputs(
        ...
        require(_idoInputInfo.totalAllocationByUser <= _idoInputInfo.totalAllocation, InvalidUserAllocation());
    }
    ...
    function _validateIdoId(uint256 idoId) internal view {
        if (idoId == 0 || idoId > idoCount) revert InvalidIdoId();
    }
